...
begin
  ...
  if p_tracking_mode not in ('count', 'cycle') then
    raise exception using errcode = '22023', message = '재고 기록 방식이 올바르지 않습니다.';
  end if;
  ...
  if p_tracking_mode = 'cycle' and coalesce(p_initial_quantity, 0) <> trunc(coalesce(p_initial_quantity, 0)) then
  ...
  if (p_package_size is null) <> (v_capacity_unit is null) then
    raise exception using errcode = '22023', message = '제품 용량과 용량 단위를 함께 입력해주세요.';
  end if;
  if p_package_size is not null and p_package_size <= 0 then
    raise exception using errcode = '22023', message = '제품 용량은 0보다 커야 합니다.';
  end if;
