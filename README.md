...
- [계절성 관찰 결정](docs/adr/0002-observe-seasonality-instead-of-manual-active-months.md): 수동 사용 월 대신 실제 기록에서 계절성을 찾기로 한 결정
- [재고 단위와 사용 관찰 분리 결정](docs/adr/0003-separate-stock-units-from-usage-observation.md): 제품 단위와 사용 기록 방식을 독립시키기로 한 결정
...
- 실제 사용 근거가 생기기 전 선택적으로 등록하는 회상 소비 기준
- 첫 완료 주기 또는 충분한 사용량 차감 기록부터 소비 속도·소진일 임시 추정
- 실제 사용 기록의 월별 분포와 계절성 자동 분석
- 반복 세일의 월·일·구매처·행사명 등록과 다음 구매 기회별 수량 추천
- 카테고리별 / 구매처별 목록 전환
...
과거 주문일을 여러 건 입력해도 현재 재고가 늘어나지 않습니다. 구매 기록은 가격·구매처·구매 간격·비축 행동을 이해하는 원자료이며 소비량 학습에는 포함하지 않습니다. 소비 속도는 실제 사용 기록을 우선하고, 실제 기록이 충분해지기 전에는 선택적으로 등록한 회상 소비 기준만 임시 근거로 사용합니다.
...
- `inventory_consumption_baselines`: 실제 관찰 전 선택적으로 쓰는 회상 소비 기준
- `inventory_product_sale_schedules`: 제품별 반복 세일 월·일, 구매처와 행사명
...
`supabase/migrations/`의 기존 적용 이력은 그대로 보존합니다. `20260831130100`·`20260831130212` migration은 회상 소비 기준, 독립적인 사용 기록 방식과 반복 세일 일정 및 새 화면이 쓰는 검증 RPC를 제공하며, `20260831130834`는 새 외래 키의 보조 인덱스를 추가합니다. 세 migration은 2026-08-31 운영 Supabase에 적용됐지만 새 프런트엔드는 아직 배포하지 않았습니다. 이전 클라이언트와 운영 데이터의 안전한 전환을 위해 기존 활성 월·단일 세일·기록 방식 열은 호환 필드로 당분간 남깁니다. 공유 Auth·workspace와 다른 앱 데이터는 건드리지 않습니다.
