...
test("같은 날 여러 번 산 기록은 구매 간격 날짜 하나로 계산한다", () => {
  ...
  const stats = calculatePurchaseStats("product-1", purchases, "2025-02-02");
...
test("과거 구매일 붙여넣기는 점·하이픈·한글 날짜를 정규화하고 중복을 제거한다", () => {
  const dates = parsePurchaseDates(
    "2024-02-10\n2024. 6. 21.\n2024년 11월 3일\n2024-02-10",
    "2025-01-01"
  );
  assert.deepEqual(dates, ["2024-02-10", "2024-06-21", "2024-11-03"]);
});

test("잘못된 날짜와 미래 날짜는 과거 구매일 입력에서 거부한다", () => {
  assert.throws(
    () => parsePurchaseDates("2024-02-30", "2025-01-01"),
    /날짜 형식/
  );
  assert.throws(
    () => parsePurchaseDates("2026-01-01", "2025-01-01"),
    /날짜 형식/
  );
});
