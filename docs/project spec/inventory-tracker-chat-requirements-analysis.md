...
> [!IMPORTANT]
> 이 문서는 2026-07-21의 코드 상태를 분석한 역사적 스냅샷이며 현재 사양이 아닙니다. 당시 없다고 기록한 구매 기록, 개수형 개봉–소진 모델, 계절 통계, 연간 필요량과 세일 구매 계획 가운데 여러 항목은 이후 구현되었습니다. 현재 동작과 규칙은 [운영·계산 규칙](../inventory-rules.md)을, 현재 기능 개요는 [README](../../README.md)를 기준으로 확인합니다. 아래 본문은 당시 문제의식과 결정 과정을 보존하기 위해 수정하지 않습니다.
...
앱의 재고 통계는 항상 단일 사용자의 1인 소비를 기준으로 비교한다. 사용 인원은 특정 개봉 제품을 실제로 함께 쓴 사람 수를 기록하는 관측값이며, 통계 기준 인원을 바꾸는 설정이 아니다.

완료된 사용 주기는 실제 사용 기간에 당시 사용 인원을 곱해 1인 환산 기간으로 바꾼다. 현재 개봉 제품의 남은 양을 추정할 때도 경과일에 현재 사용 인원을 곱해 이미 소비한 1인 환산 일수를 차감한다.
...
따라서 일시적으로 2명이 함께 사용한 기간은 소비 속도가 두 배였다는 사실만 반영하고, 월평균 소비량·연간 필요량·소진 예측·구매 추천은 계속 1인 기준으로 유지한다.
